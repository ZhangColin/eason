...
CREATE TABLE `gds_products` (
  ...
  `picture_url` varchar(256) NOT NULL COMMENT '商品图片',
...
CREATE TABLE `gds_product_details` (
  `id` bigint NOT NULL COMMENT '商品Id',
  `place` varchar(256) NOT NULL COMMENT '商品图片',
  `description` varchar(256) NOT NULL COMMENT '商品图片',
  `brand` varchar(256) NOT NULL COMMENT '商品图片',
  `weight` varchar(256) NOT NULL COMMENT '商品图片',
  `specification` varchar(256) NOT NULL COMMENT '商品图片',
  `picture_url` varchar(256) NOT NULL COMMENT '商品图片',
  `created` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `updated` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`) USING BTREE
